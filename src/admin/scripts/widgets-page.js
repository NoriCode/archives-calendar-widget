jQuery(function ($) {
	...
	$widgetsRight.on('click', '.arcw-accordion .accordion-section:not(.open)', function () {
		...
		if ($(this).parent().find('.accordion-section.open').length)
			$(this).parent().find('.accordion-section.open .accordion-section-content').slideUp('fast', function () {
				$(this).parent().removeClass('open')
			});
		$(this).find('.accordion-section-content').slideDown('fast', function () {
			$(this).parent().addClass('open')
		});
	})
		.on('click', '.arcw-accordion .accordion-section.open .accordion-section-title', function () {
			$(this).parent().find('.accordion-section-content').slideUp('fast', function () {
				$(this).parent().removeClass('open')
			});
		});
